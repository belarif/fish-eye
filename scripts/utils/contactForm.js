...
function validateMessage() {
  const removeErrorMessage = document.querySelector(
    ".modal div:nth-child(5) span"
  );

  if (
    !messageElt.value.match(
    ...
    const errorMessage =
      "Votre message doit etre compris entre 20 et 700 caractères";
    const spanErrorMessage = document.querySelectorAll(
      ".modal div:nth-child(5) span"
    );
...
function displaySuccessMessage() {
  successMessageElt.textContent =
    "Votre message ainsi vos coordonnées sont affichés avec succès dans la console";
